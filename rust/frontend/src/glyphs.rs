...
// resources/images, embedded in the binary. Rendering at the exact
// display pixel size keeps them crisp at every resolution. Keys are the
// resource path without the extension ("icons/ScrollUp", "status/NFC",
// "buttons/style_a/FaceEast"). Every source is authored white on
// transparent; the UI tints with `colorize`. The two-tone favorite heart
// is the exception: its white fill and black keyline map to the theme's
// marker roles before rasterization.
...
const SOURCES: &[(&str, &str)] = &[
    ...
    // Prompts", see LICENSES/Kenney-ATTRIBUTION.txt).
    (
...
/// paths. The Hub action tiles make their own choices.
...
///
/// Generated rather than baked: the frontend rasterizes from a fixed
/// source list with no atlas, and a mask is always exactly as wide as the
/// radius it cuts, so `px` is the radius and one key serves every tier.
/// Even-odd fill takes the disc out of the square without needing to
/// describe the concave wedge directly.
...
/// in `tint`. The color is baked into the artwork rather than applied by
/// the view: the renderer then draws a plain image, with no per-item
/// offscreen colorize pass and no second texture per tile. A transparent
/// tint keeps the artwork's own colors (the two-tone heart).
