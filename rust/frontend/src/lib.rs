...
#[cfg_attr(
    ...
    allow(
        dead_code,
        reason = "the SMBus probe is MiSTer-only; the desktop build reads the power-supply class instead"
    )
)]
mod mister_battery;
mod navigation;
...
/// Categories Core surfaces but the frontend doesn't expose. User-hidden
/// categories are a separate, durable preference (`hidden_categories` in
/// the router state).
...
/// Merge frontend.toml's durable settings over the tmpfs state snapshot;
/// the config file wins at startup.
...
/// Wall-clock readout for the header, ticking every 30 seconds
/// (minutes-only display never needs finer).
/// The flag follows `clock_twelve_hour` (the persisted `clock_format`
/// setting, with `auto` following the language or host locale) and is
/// shared with the Settings toggle so format changes apply live.
fn start_clock(
    app: &App,
...
mod tests {
    ...
    fn durable_display_config_overrides_tmpfs_state() {
        ...
        // the point is that a wild config value is clamped rather than
        // persisted verbatim, not what those bounds happen to be today.
