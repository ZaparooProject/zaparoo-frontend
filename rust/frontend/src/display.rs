...
//! Display policy for the frontend. Output timing, framebuffer render
//! size, and CRT scene size are separate quantities.
...
mod tests {
    ...
    #[allow(
        clippy::float_cmp,
        reason = "integer font roles must match the sizing golden fixture exactly"
    )]
    fn hdmi_font_roles_use_the_integer_ladder_while_crt_keeps_bitmap_type(
    ) -> Result<(), slint::PlatformError> {
        use slint::ComponentHandle;
        assert!(
            slint::platform::set_platform(Box::new(crate::route_motion::ProbePlatform)).is_ok()
