...
/// The root category grid's band and cell fit, on the shared grid
/// geometry: the Hub's own insets and gaps, its band from the status
/// strip to a bottom margin that clears the active label, and cells
/// capped at the Hub's resolved tile size so both grids read as the same
/// object.
...
pub fn handle_action(ctx: &Ctx, app: &App, action: &str) {
    ...
    match action {
        ...
        // so a long option list never has to be cycled blind.
