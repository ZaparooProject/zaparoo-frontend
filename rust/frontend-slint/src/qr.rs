...
// module plus the standard 4-module quiet zone - and the UI scales it
// to the largest whole multiple with pixelated rendering, so modules
// stay square at every output size.
//
// The two colors come from the palette rather than being black and
// white (`QrMatrix.qml`). Every preset derives a `qr_light` and a
// `qr_dark` from its own accent, held to the light and dark ends of the
// lightness range so the contrast a scanner needs survives the theming;
// see `zaparoo_app::palette` and docs/style.md -> "Themed QR codes".
// The quiet zone is part of the code, so it takes `qr_light` too -- a
// white border around a tinted matrix is the one thing that would break
// a scan.
