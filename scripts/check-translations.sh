...
#
# Fail if the committed gettext template is stale against the `@tr()`
# strings in the .slint files. Source locations are part of the template,
# so any .slint edit that moves a translatable string must be followed by
# `just tr-extract`.
set -euo pipefail
...
"$script_dir/extract-translations.sh" "$fresh"
...
if ! diff -u <(grep -v '^"POT-Creation-Date' "$pot") <(grep -v '^"POT-Creation-Date' "$fresh"); then
    echo "rust/frontend/translations/frontend.pot is stale; run \`just tr-extract\` and commit the result." >&2
    exit 1
fi
