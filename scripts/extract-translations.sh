...
#
# Regenerate the frontend's gettext template from every .slint file.
...
out="${1:-$repo_root/rust/frontend/translations/frontend.pot}"

cd "$repo_root"
mapfile -t files < <(find rust/frontend/ui -name '*.slint' | sort)
