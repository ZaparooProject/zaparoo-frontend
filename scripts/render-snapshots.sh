...
#
# Render the frontend's screens offline through the software renderer (the
# same pipeline the MiSTer build presents through) at 240p CRT, 540p, 720p
# and 1080p. Output lands in output/snapshots/<screen>-<w>x<h>.png. Pass a
# language (e.g. `de`) as the first argument to render through a bundled
# translation.
set -euo pipefail
