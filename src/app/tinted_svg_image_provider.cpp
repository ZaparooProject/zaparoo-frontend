...
bool parseRequest(const QString& id, Request* out, QString* error)
{
    ...
    // System logos, Hub category icons, UI glyphs, host-status icons, and
    // help-bar controller/keyboard button glyphs all share the provider.
    // Corner masks (ContextMenu's rounded scrim hole) share it too, but have
    // no SVG source -- they are baked directly from a Qt Quick render -- so
    // they are exempt from the .svg suffix check below.
    ...
    const bool knownPrefix = out->resourcePath.startsWith(QStringLiteral("images/systems/")) ||
                             ...
                             out->resourcePath.startsWith(QStringLiteral("images/buttons/")) ||
                             cornerPrefix;
