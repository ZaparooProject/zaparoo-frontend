...
// Rust FFI: check that an absolute path is inside the configured override dir.
// Returns false if the override dir is unset (feature off) or the path escapes.
extern "C" bool zaparoo_system_image_is_in_override_dir(const uint8_t* path, size_t len);
...
SystemImageProvider::SystemImageProvider() : QQuickImageProvider(QQuickImageProvider::Image) {}

QImage SystemImageProvider::requestImage(const QString& id, QSize* size, const QSize& requestedSize)
{
    // `id` is the absolute path to the override file, passed verbatim from the
    // `system-image/` cover key emitted by `system_image_overrides::override_path`.
