...
# Resolve a Cargo package's source root from the workspace lockfile. This lets CMake consume
# non-Rust package assets (QML, icons, CMake helpers) from either a workspace path dependency or a
# crates.io dependency without hardcoding rust/<crate-name>/ paths.
function(zaparoo_find_cargo_package_root package_name out_var)
    find_program(_zaparoo_cargo_executable NAMES cargo REQUIRED)

    execute_process(
        COMMAND "${_zaparoo_cargo_executable}" metadata --format-version 1 --locked --manifest-path
    ...
    if(_zaparoo_package_count GREATER 0)
        ...
        foreach(_zaparoo_index RANGE 0 ${_zaparoo_last_package})
            string(
                JSON
                _zaparoo_current_name
                GET
                "${_zaparoo_cargo_metadata}"
                packages
                ${_zaparoo_index}
                name
            )
            if(_zaparoo_current_name STREQUAL package_name)
                string(
                    JSON
                    _zaparoo_current_manifest_path
                    GET
                    "${_zaparoo_cargo_metadata}"
                    packages
                    ${_zaparoo_index}
